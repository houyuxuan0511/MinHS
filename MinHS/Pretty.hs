{-# OPTIONS -Wno-missing-signatures -Wno-orphans #-}
...
instance Pretty Bind where
  pretty (Bind n ty ps expr) = string n <+> symbol "::" <+> pretty ty
                                        <+> params (symbol "=" <+> pretty expr)
...
instance Pretty Type where
  pretty = prettyType'
    where
      ...
      prettyType' t@(_ `Arrow` _) = parens (prettyType t)
      prettyType' t = prettyType t
      prettyType'' t@(TypeApp _ _) = parens (prettyType' t)
      prettyType'' t = prettyType' t
...
instance Pretty TypeError where
   ...
   pretty (NoSuchConstructor x) =  err "Constructor"
                               <+> string x
                               <+> err "not in scope."
   pretty (TypeConstructorSaturated t _k) =  err "Expected"
                                         <+> pretty t
                                         <+> err "to be a type constructor, but it is an atomic type."
   pretty (KindMismatch _k1 _k2 t) =  err "Type term"
                                  <+> pretty t
                                  <+> err "is not valid."
